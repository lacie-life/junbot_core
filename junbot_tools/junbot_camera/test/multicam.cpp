...
int main( int argc, char** argv ) try
{
    ...
    std::map<std::string, rs2::colorizer> color_map;

    // Declare RealSense pipeline, encapsulating the actual device and sensors
    std::vector<rs2::pipeline> pipelines;

    std::vector<rs2::pipeline_profile> selections;
    ...
    for (auto&& serial : serials)
    {
        ...
        color_map[serial] = rs2::colorizer();
    }
    ...
    std::string model= "/home/hn/junbot_ws/src/JunBot/junbot_tools/junbot_camera/Model/yolov5s.engine";
    ...
    rs2::stream_profile  depth_stream = selections.at(0).get_stream(RS2_STREAM_DEPTH);
    rs2::stream_profile  color_stream = selections.at(0).get_stream(RS2_STREAM_COLOR);
    ...
    auto sensor = selections.at(0).get_device().first<rs2::depth_sensor>();
    float depth_scale =  sensor.get_depth_scale();

    auto depth_min = 0.11; //meter
    auto depth_max = 1.0; //meter
    rs2::video_stream_profile  depth_stream_ = selections.at(0).get_stream(RS2_STREAM_DEPTH).as<rs2::video_stream_profile>();
    rs2_intrinsics depth_intrin = depth_stream_.get_intrinsics();
    rs2::video_stream_profile  color_stream_ = selections.at(0).get_stream(RS2_STREAM_COLOR).as<rs2::video_stream_profile>();
    ...
    while ((ros::ok()))
    {
        ...
        {
            rs2::frameset data = fs;
            rs2::frame rgb = data.get_color_frame().apply_filter(color_map.at(0));
            rs2::frame depth = data.get_depth_frame().apply_filter(color_map.at(0));
            ...
            for(auto object:res) {
                ...
                cv::putText(frame_rgb, std::to_string((float)distance_min), cv::Point(r.x, r.y +20), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            }
            if (check_obs == 0)
            {
                std::string tmp = "1";
