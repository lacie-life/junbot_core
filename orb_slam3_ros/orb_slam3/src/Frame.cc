...
namespace ORB_SLAM3 {
    ...
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
                 const double &timeStamp, ORBextractor *extractor, line_lbd_detect* line_lbd_ptr_frame, 
                 ORBVocabulary *voc, cv::Mat &K, cv::Mat &distCoef,
                 const float &bf, const float &thDepth, GeometricCamera *pCamera, Frame *pPrevF,
                 const IMU::Calib &ImuCalib, const std::vector<BoxSE> & bbox)
            : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
              mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
              mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf),
              mThDepth(thDepth),
              ...
              mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
              mpCamera(pCamera), mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false),
              mColorImage(imColor.clone()), mQuadricImage(imColor.clone()) , 
              mpline_lbd_ptr_frame(line_lbd_ptr_frame), boxes(bbox) {
        // Frame ID
        mnId = nNextId++;
    ...
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
                 const cv::Mat &imMask, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc,
...
// Camera parameters can be raised separately to make a class
    void Frame::ComputeImageBounds() {
        ...
        if (mDistCoef.at<float>(0) != 0.0) {
            ...
            mat.at<float>(1, 0) = Camera_width;
            mat.at<float>(1, 1) = 0.0;
            mat.at<float>(2, 0) = 0.0;
            mat.at<float>(2, 1) = Camera_height;
            mat.at<float>(3, 0) = Camera_width;
            mat.at<float>(3, 1) = Camera_height;
