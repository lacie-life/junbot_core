...
namespace semantic_slam
{
                                       ...
                                       vector<cv::Point2f>& points_current)
    {
        ...
        // Rotation Histogram (to check rotation consistency)
        // Matching points Observation direction difference Histogram Statistics Used to filter the best match
        vector<int> rotHist[HISTO_LENGTH];
        for(int i=0;i<HISTO_LENGTH;i++)
            rotHist[i].reserve(500);
        const float factor = 1.0f/HISTO_LENGTH;
        ...
        const cv::Mat Rlw = _LTcw.rowRange(0,3).colRange(0,3);
        const cv::Mat tlw = _LTcw.rowRange(0,3).col(3);
        const cv::Mat tlc = Rlw*twc + tlw;// The translation vector from the current frame to the previous frame

        // Judging whether to move forward or backward
        const bool bForward = tlc.at<float>(2) > CurrentFrame.mb && !bMono;
        // For non-monocular cases, if Z>0 and greater than the baseline, it means moving forward

        const bool bBackward = -tlc.at<float>(2) > CurrentFrame.mb && !bMono;
        // For non-monocular cases, if Z<0, and the absolute value is greater than the baseline, it means moving forward

        // Step 2: Traverse all key points of the previous frame (corresponding to map points)
        for(int i=0; i<LastFrame.N; i++)
        {
            ...
            {
                // The map point is not an outer point, but an inner point,
                // a point of compound transformation relationship
                if(!LastFrame.mvbOutlier[i])
                {
                    // Project
                    ...
                    // When moving forward, the area of the dot increases, and it is a feature point at a certain scale m. Due to the increased area, it needs to be detected at a higher scale
                    // Therefore m>=n, corresponding to the forward situation, nCurOctave>=nLastOctave. The backward situation can be analogized
                    ...
                    // Step 5: traverse the candidate key points, calculate the match with the map point descriptor,
                    // calculate the distance, and keep the closest match
                    const cv::Mat dMP = pMP->GetDescriptor();
                    int bestDist = 256;
                    int bestIdx2 = -1;
                    ...
                    if(bestDist<=TH_HIGH)
                    {
                        ...
                        points_last.push_back( LastFrame.mvKeys[i].pt );
                        points_current.push_back( CurrentFrame.mvKeys[bestIdx2].pt );
        ...
        // Step 6: Eliminate incorrectly matched points according to the direction difference consistency constraint
        //Apply rotation consistency
        if(mbCheckOrientation)
        {
            int ind1=-1;
            int ind2=-1;
            int ind3=-1;
            // The three bins with the highest statistical histogram are retained, and the matching points in other ranges are eliminated.
            // In addition, if the highest is more than 10 times higher than the second highest, only the matching points in the highest bin will be kept.
            // If the highest is more than 10 times higher than the third highest, keep the matching points between the highest and the second highest bin.
            ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);
